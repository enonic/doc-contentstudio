...
Read more about content preview modes in the <<workbench/preview#,Preview>> documentation.
...
:imagesdir: workbench/navigator/images
...
Default Online From time in the Publishing Wizard can now be configured in Content Studio's <<config#default_publish_from_time,config file>>. If no config is set, the default time will be set to 12:00.
...
Search results produced by the Navigator's <<workbench/navigator#search_panel,Search panel>> can now be exported into a CSV file using the download icon next to the Search Panel's free-text search field. The file will contain the following fields: *Id*, *Path*, *Creator*, *Modifier*, *Created*, *Modified*, *Owner*, *Content Type*, *Display Name*, *Language*, *Published From*, *Published Until*, *First Published*.
