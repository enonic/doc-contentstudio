...
Watch this overview for a guided tour of Content Studio 6 and its redesigned interface.

.Content Studio 6 overview
video::n_RmUz_jGas[youtube]
...
The Version history panel has been updated to take advantage of the new features in XP8.
...
* *Patch handling* — The Version history panel informs users when a version is a patch, and prevents reverting to versions that may break content due to missing required fields or other issues.
...
* *Content import* — importing content is now a native Content Studio feature.
...
The list below reflects Content Studio 6.0.3. Several issues reported for 6.0.0 have been resolved in the 6.0.x patch releases — see the https://github.com/enonic/app-contentstudio/releases[release changelogs] for the full list. Remaining known issues that we are working to resolve ASAP:

* *Archive log message* — the Delete/Archive dialog is missing the input for providing a log message for the archived version. Archiving with a message is therefore not available in 6.0.
* *Publishing wizard* — a parent item is not automatically checked when one of its children is selected in the dependencies list.
* *Stale data* - for some changes to data the UI does not update immediately, but requires a manual refresh.
* *Fragments* — preview rendering for fragments is not always correct.
* *Arrow keys* - for some input fields and lists, the arrow keys do not work as expected.

Resolved in the 6.0.x patch releases:

* *Publishing wizard* — missing dependencies and the missing "select all" option for dependencies (6.0.1, 6.0.2).
* *Image editor* — cropping (6.0.1, 6.0.2).
* *Image selector* — drag and drop (6.0.2).
* *Fragments* — incorrect menu items (6.0.2, 6.0.3).
* *Validation on rename* — the rename dialog now validates the input (6.0.2, 6.0.3).
* *Version history* — wrong dates for imported content (6.0.1).
* *Content icons* — custom icons can be uploaded for content items and projects (6.0.3).
