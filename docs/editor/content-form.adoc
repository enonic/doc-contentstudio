...
It is also possible for the Display Name to be generated dynamically from the values of other form
fields, if the application's content type defines a Display Name expression.
...
transliterated into their english equivalents. It's possible to turn off this behaviour in the Content Studio's <<../config#disable_path_name_transliteration, config file>>.
...
Selecting a component in the tree will highlight the component in the Page editor and display its configuration in the <<page-editor#_components_widget,Components widget>>.
