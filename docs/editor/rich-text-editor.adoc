...
Rich Text Editor is used by the https://developer.enonic.com/docs/cms/stable/schemas/form-items/htmlarea[HTMLArea input type]
and for editing <<page-editor#text,Text components>> inside the Page editor.
...
Additionally, there's an option to apply <<#custom_image_styles,custom image styles>>.
...
The *Accessibility* section at the bottom lets you mark the image as decorative, so assistive technologies can ignore it, or provide alternative text that describes the image's meaning or purpose.
...
Custom Image Styles can be defined in your application as https://developer.enonic.com/docs/cms/stable/richtext/styles[described] in the CMS section of XP docs.
