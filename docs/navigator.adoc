 ...
 * If selected content is not published and current user doesn't have publishing permissions: *Request Publishing* (opens the <<workflow#publish_request,Publish Request>> wizard)
...
This panel displays the list of contextual widgets which can be used to work with the selected content. Widgets are applications that expand the functionality of Content Studio. See the <<context-panel#,Context Panel>> reference for the built-in widgets and how custom widgets plug in.
