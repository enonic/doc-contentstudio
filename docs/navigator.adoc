...
* *Archive:* Opens the <<archive#,Archive item(s)>> dialog for the selected content.
 ...
 * If selected content is not published: *Publish* (opens the <<workflow/publish#_publishing_wizard,Publishing wizard>> dialog)
 * If selected content is published: *Unpublish* (opens the <<workflow/unpublish#,Unpublish Wizard>> dialog for taking the content offline)
 * If selected content is published but has child items that are not published: *Publish Tree* (opens the <<workflow/publish#_publish_tree,Publishing wizard>> dialog with selected content and all descendants)
 * If selected content has workflow state "In progress": *Mark as Ready* (to mark selected content item(s) as ready for publishing)
 * If selected content is not published and current user doesn't have publishing permissions: *Request Publishing* (opens the <<workflow/publish-request#,Publish Request>> wizard)
...
The *Context panel* is the multi-function column on the right of the Content Navigator (and the <<editor#,Content Editor>>). It hosts a set of *widgets* — small tools that act on the currently selected content item. Widgets can be built-in, shipped by https://market.enonic.com/vendors/enonic/content-studio-plus[Content Studio+], or contributed by any installed application.

image::navigator-context-panel.png[Context panel, 50%, align="center"]

The panel is toggled open and closed with the image:navigator-context-panel-icon.png[title=Context panel toggle, width=24px] icon next to the Action-menu button, or (in the Content Editor) with the cog icon on the right of the editor toolbar.
...
The *Details* widget shows the basic properties of the selected content item — the core metadata an editor typically needs to know without opening the item.
...
==== Versions, Dependencies, Components

* The *Version history* widget lists every version of the selected item and lets you compare or revert. See <<editor/versions#,Versions>>.
* The *Dependencies* widget shows the inbound and outbound references between the item and other content. See <<navigator/dependencies#,Dependencies>>.
* The *Components* widget is the page-building tool that appears in the Content Editor for renderable content. See <<editor/page-editor#_components_widget,Page editor>>.
