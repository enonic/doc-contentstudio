...
Workflow state of the content is displayed via corresponding icon, except for "Published" state when no icon is used. Transitions between states are driven by a handful of publishing actions: *Mark as ready*, *Request publishing*, *<<workflow/publish#,Publish>>*, and *<<workflow/unpublish#,Unpublish>>*.
